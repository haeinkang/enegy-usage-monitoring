...
const airQualSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(getAirQualData.fulfilled, (state, action) => {
      state.data = action.payload;
      state.loaded = true;
      state.error = null;
    });
    builder.addCase(getAirQualData.rejected, (state, payload) => {
      state.loaded = true;
    ...
    try {
      ...
      const res: AirQualByLclgvNumeric[] = _(items)
        ...
        .map((items, lclgvNm) => {
          ...
          const averageValues = reduce(
            metrics,
            (acc, key) => {
              return {
                ...acc,
                [key]: Number(
                  meanBy(items, (o) => parseFloat(o[key]) || 0).toFixed(1)
                ),
              };
            },
            {}
...
export const { selectLclgvNm } = airQualSlice.actions;
