...
function LeftPanel() {
  ...
  const selectedAirQual = useSelector(
    (state: RootState) => state.airQual.selected
  );

  return (
    <GridContainer>
      <Panel square elevation={10} collapsed={`${isCollapsed}`}>
        {selectedAirQual ? <DetailedAirQuality /> : <GasUsageRank />}
      </Panel>
...
const Panel = styled(Paper)<{ collapsed: "true" | "false" }>`
  ...
  width: ${(props) =>
    props.collapsed === "true" ? "0" : "calc(10vw + 24rem)"};
...
const CollapseButton = styled(Button)<{ collapsed: "true" | "false" }>`
  ...
  background: ${(props) =>
    `var(--joy-palette-neutral-${
      props.collapsed === "true" ? "700" : "800"
    })`} !important;
  svg {
    color: #fff;
  }
