...
const MapChart = () => {
  ...
  const gasDataLoaded = useSelector(
    (state: RootState) => state.gasUsage.loaded
  );
  const gasUsageList = useSelector((state: RootState) => state.gasUsage.data);
  const clickedItem = useSelector(
    (state: RootState) => state.gasUsage.clickedItem
  );
  ...
  const leftPanelCollapsed = useSelector(
    (state: RootState) => state.leftPanel.isCollapsed
  );
  ...
  useEffect(() => {
    const initializeMap = async () => {
      try {
        ...
        console.error(
          "Google Maps API 또는 ECharts 초기화 중 오류 발생:",
          error
        );
  ...
  const updateEChartOpts = () => {
    try {
      if (chartRef.current) {
        ...
        if (chart) {
          ...
          chart.on("click", (params: any) => {
            const clickedItem = find(
              gasUsageList,
              (o) => o.lclgvNm === params.name
            );
      ...
      console.error(
        "Echart에 맞게 가공한 데이터 Echart에 업데이트하는 과정에서 오류 발생",
        e
      );
    }
  };
  ...
  const smoothZoomTo = (
    ...
    if (chart) {
      ...
      const initialZoomOption = {
        gmap: {
          center: leftPanelCollapsed
            ? newCoord
            : [newCoord[0] - 0.05, newCoord[1]],
          zoom: initialZoom,
        },
      };
      ...
      setTimeout(() => {
        const finalZoomOption = {
          gmap: {
            center: [newCoord[0] - 0.05, newCoord[1]] as GeoCoord,
            zoom: finalZoom,
          },
        };
        updateSelectedItemStyle(chart, finalZoomOption);
