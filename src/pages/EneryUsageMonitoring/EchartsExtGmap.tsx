...
const EchartsExtGmap = () => {
  ...
  const gasLoaded = useSelector((state: RootState) => state.gasUsage.loaded);
  const airLoaded = useSelector((state: RootState) => state.airQual.loaded);
  ...
  useEffect(() => {
    if (gasLoaded && airLoaded && googleLoaded && chartRef.current) {
